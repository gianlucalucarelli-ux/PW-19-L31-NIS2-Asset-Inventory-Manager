-- =========================================================================
-- FILE: 01_schema_3nf.sql (VERSIONE DEFINITIVA EMENDATA)
-- DESCRIZIONE: DDL Schema 3NF e logica di Audit Trail con correzione FK
-- =========================================================================

-- Rimozione preventiva delle tabelle per garantire l'atomicità dello script
...
CREATE TABLE public.responsabile (
    ...
    CONSTRAINT fk_responsabile_organizzazione FOREIGN KEY (organizzazione_id) 
        REFERENCES public.organizzazione(id) ON DELETE RESTRICT
);
...
CREATE TABLE public.asset (
    ...
    CONSTRAINT fk_asset_organizzazione FOREIGN KEY (organizzazione_id) 
        REFERENCES public.organizzazione(id) ON DELETE RESTRICT,
    CONSTRAINT fk_asset_responsabile FOREIGN KEY (responsabile_id) 
        REFERENCES public.responsabile(id) ON DELETE RESTRICT
);
...
CREATE TABLE public.servizio (
    ...
    organizzazione_id INT NOT NULL,
    responsabile_id INT NOT NULL,
    CONSTRAINT fk_servizio_organizzazione FOREIGN KEY (organizzazione_id) 
        REFERENCES public.organizzazione(id) ON DELETE RESTRICT,
    CONSTRAINT fk_servizio_responsabile FOREIGN KEY (responsabile_id) 
        REFERENCES public.responsabile(id) ON DELETE RESTRICT
);
...
-- 7. Entità Associativa N:M (Responsabili e Ruoli Normativi)
CREATE TABLE public.responsabile_ruolo (
    ...
    CONSTRAINT fk_resp_ruolo_responsabile FOREIGN KEY (responsabile_id) 
        REFERENCES public.responsabile(id) ON DELETE CASCADE,
    CONSTRAINT fk_resp_ruolo_ruolo FOREIGN KEY (ruolo_id) 
        REFERENCES public.ruolo(id) ON DELETE CASCADE
);

-- 8. Entità Dipendenza (CORRETTA)
CREATE TABLE public.dipendenza (
    ...
    fornitore_id INT,
    descrizione TEXT,
    CONSTRAINT fk_dipendenza_asset FOREIGN KEY (asset_id) 
        REFERENCES public.asset(id) ON DELETE CASCADE,
    CONSTRAINT fk_dipendenza_servizio FOREIGN KEY (servizio_id) 
        REFERENCES public.servizio(id) ON DELETE SET NULL,
    CONSTRAINT fk_dipendenza_fornitore FOREIGN KEY (fornitore_id) 
        REFERENCES public.fornitore(id) ON DELETE SET NULL
);

-- 9. Entità Versioning Asset (Tabella di Audit)
CREATE TABLE public.versioning_asset (
    ...
    descrizione TEXT,
    CONSTRAINT fk_versioning_asset FOREIGN KEY (asset_id) 
        REFERENCES public.asset(id) ON DELETE CASCADE
);

-- =========================================================================
-- TRIGGER E FUNZIONE IN PL/pgSQL
-- =========================================================================
