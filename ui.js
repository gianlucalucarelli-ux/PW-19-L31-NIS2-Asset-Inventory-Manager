...
const safeLocalStorage = {
    get(key) {
        try { return window.localStorage.getItem(key); }
            ...
            console.info('Preferenza locale non disponibile; uso dei valori predefiniti.');
            return null;
        }
    },
    set(key, value) {
        try { window.localStorage.setItem(key, value); }
        catch (error) { /* La preferenza resta valida per la sessione corrente. */ }
    },
    remove(key) {
        try { window.localStorage.removeItem(key); }
        catch (error) { /* Nessuna azione necessaria. */ }
    }
};
...
function initializeSidebarNavigation() {
    ...
    const savedGroup = safeLocalStorage.get(SIDEBAR_GROUP_STORAGE_KEY);
    ...
    groups.forEach((group) => {
        ...
        toggle.addEventListener('click', () => {
            ...
            if (willExpand) safeLocalStorage.set(SIDEBAR_GROUP_STORAGE_KEY, group.dataset.menuGroup || '');
            else safeLocalStorage.remove(SIDEBAR_GROUP_STORAGE_KEY);
        });
    });
}
...
function revealNavigationGroupForRoute(route) {
    ...
    safeLocalStorage.set(SIDEBAR_GROUP_STORAGE_KEY, activeGroup.dataset.menuGroup || '');
