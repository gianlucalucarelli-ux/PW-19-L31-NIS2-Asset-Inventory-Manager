...
} from './ui.js?build=20260730-f8';
import {
    initializeRouter,
    navigateTo,
    refreshCurrentRoute
} from './router.js?build=20260730-f6';
import {
    signIn,
    getCurrentSession,
    resolveAccessState,
...
const safeSessionStorage = {
    ...
    set(key, value) {
        ...
        catch (error) { /* L'audit resta protetto dal controllo lato database. */ }
    }
};
...
async function registerAuthorizedSessionAccess(session, accessState) {
    ...
    if (safeSessionStorage.get(storageKey) === 'registrato') return;
    ...
    try {
        ...
        if (auditId !== null && auditId !== undefined) {
            safeSessionStorage.set(storageKey, 'registrato');
        }
    } finally {
