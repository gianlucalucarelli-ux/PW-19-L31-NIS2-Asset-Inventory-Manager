...
Output:
- Core_Migration_Pipeline_01-30.png/.svg
- Diagnostic_Validation_Patch_Toolkit_X1-X21.png/.svg
"""
...
def setup(title: str, subtitle: str, figsize=(13.5, 9.2)):
    fig, ax = plt.subplots(figsize=figsize)
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
    ax.axis("off")
    fig.patch.set_facecolor("white")
    ax.text(0.5, 0.965, title, ha="center", va="top", fontsize=18,
            fontweight="bold", color=PALETTE["text"], family="DejaVu Sans")
    ax.text(0.5, 0.925, subtitle, ha="center", va="top", fontsize=10.5,
            color="#52636E", family="DejaVu Sans")
    return fig, ax


def box(ax, xy, wh, title, lines, face, title_size=10.0, body_size=7.7):
    x, y = xy
    w, h = wh
    ...
    ax.add_patch(patch)
    ax.text(x + w/2, y + h - 0.038, title, ha="center", va="top",
...
def orth_arrow(ax, points):
    """Draw an orthogonal connector, adding the arrow head on the last segment."""
    for a, b in zip(points[:-2], points[1:-1]):
        ax.plot([a[0], b[0]], [a[1], b[1]], color=PALETTE["arrow"], linewidth=1.25)
...
def core_pipeline():
    ...
    specs = [
        ...
        (1, 0, "FASE 2 - VISTE, AUDIT E TEMPO", ["05 Viste ACN", "06 Audit asset", "07 Time-zone", "08 Policy di lettura"], PALETTE["green"]),
        ...
        (0, 2, "FASE 7 - CONSERVAZIONE E CHIUSURA", ["20 Archiviazione logica", "21 Eliminazione ultimo CASCADE"], PALETTE["slate"]),
        (1, 2, "FASE 8 - ACCESSO OPERATIVO", ["22 Accesso valutazione asset", "23 Policy sulle 14 tabelle"], PALETTE["mint"]),
        (2, 2, "FASE 9 - QUALITA DEI DATI", ["24 Archivio asset dimostrativi", "25 Terminologia neutrale", "26 Contatto normalizzato"], PALETTE["gold"]),
    ]
    centers = {}
    for c, r, title, lines, face in specs:
        x, y = xs[c], ys[r]
        box(ax, (x, y), (w, h), title, lines, face)
    ...
    x, y, fw, fh = 0.19, 0.055, 0.62, 0.115
    box(ax, (x, y), (fw, fh), "FASE 10 - GOVERNANCE, ACCESSI E ASSESSMENT FNCSDP",
        ...
        PALETTE["fncsdp"], title_size=10.6, body_size=8.0)
    ax1, ay1 = centers[(2,2)]
    mid_y = (ay1 - h/2 + y + fh) / 2
    orth_arrow(ax, [
        (ax1, ay1-h/2+0.005),
        (ax1, mid_y),
        (x+fw/2, mid_y),
...
def diagnostic_toolkit():
    ...
    specs = [
        (0.05, 0.64, 0.40, 0.21, "ISPEZIONE STRUTTURALE E SICUREZZA",
        ...
        (0.55, 0.64, 0.40, 0.21, "BONIFICA E PROTOTIPAZIONE CONTROLLATA",
         ["X3 allineamento servizi", "X4 dataset Supply Chain", "X5 prototipo audit JSON"], PALETTE["sand"]),
        (0.05, 0.36, 0.40, 0.21, "READINESS DEL MODELLO",
        ...
        (0.55, 0.36, 0.19, 0.21, "CHIUSURA STRUTTURALE",
         ["X17 verifica finale", "backend superata"], PALETTE["lav"]),
        (0.76, 0.36, 0.19, 0.21, "ACCESSI E QUALITA",
         ["X18 AAL/MFA", "X19 dati e anomalie"], PALETTE["mint"]),
        (0.05, 0.105, 0.40, 0.18, "AUDIT DEGLI ACCESSI APPLICATIVI",
         ["X20 verifica LOGIN, LOGOUT e MFA_VERIFICATA", "Privilegi, contesto utente e coerenza dell'Audit Log"], PALETTE["rose"]),
        (0.55, 0.105, 0.40, 0.18, "VALIDAZIONE ASSESSMENT FNCSDP",
         ["X21 verifica 36 tabelle e 11 viste", "3FN, RLS, trigger, mapping, misure, score e gap"], PALETTE["fncsdp"]),
    ]
    for x,y,w,h,title,lines,face in specs:
        box(ax,(x,y),(w,h),title,lines,face,title_size=10.2,body_size=7.8)
    arrow(ax,(0.45,0.745),(0.55,0.745))
    arrow(ax,(0.25,0.64),(0.25,0.57))
    orth_arrow(ax, [(0.75,0.64),(0.75,0.605),(0.645,0.605),(0.645,0.57)])
    arrow(ax,(0.45,0.465),(0.55,0.465))
    arrow(ax,(0.74,0.465),(0.76,0.465))
    orth_arrow(ax, [(0.645,0.36),(0.645,0.32),(0.25,0.32),(0.25,0.285)])
    orth_arrow(ax, [(0.855,0.36),(0.855,0.32),(0.75,0.32),(0.75,0.285)])
